...
def new_game():
    global W, H, count, image_to_puzzle_current, lose, index
    text = ''
    index = 0
    count_use = 0
    used = False
            ...
            for e in event.get():
                ...
                if e.type == USEREVENT and e.button == scroll_down_button:
                    ...
                    if index + 1 >= len(index_list_copy):
                        ...
                        index += 1
                    if (index - 1 < 0) and index_list_copy[index - 1] in [bool_type[i] for i in bool_type.keys()]:
                        index2 = len(index_list_copy) - 1
                        del index_list_copy[index2]
                    elif (index - 1 >= 0) and index_list_copy[index - 1] in [bool_type[i] for i in bool_type.keys()]:
                        del index_list_copy[index - 1]
                    if index + 1 >= len(index_list_copy):
                        index = 0
                    if len(index_list_copy) != 0:
                        current_image = image.load(f'temp_folder/square_{index_list_copy[index]}.jpg')
                    else:
                        text = 'No pics in order'
                        text_surface = font_to_warning.render(text, True, Color('RED'))
